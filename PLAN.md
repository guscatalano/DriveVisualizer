...
- **Alternate views.** ✅ DONE. View picker (persisted): Treemap, Sunburst (radial rings, center = root, click-center zooms out), Icicle (flame-graph rows with inline labels). Folders in the new views are tinted by their dominant content category (bottom-up tally in Core, tested); files keep exact category colors; filter ghosting and selection sync work in all modes.
