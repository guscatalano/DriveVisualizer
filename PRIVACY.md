...
- **Optional MCP server (off by default).** If you start it from Settings, the app listens for connections from other programs on *your own machine only* (`localhost`) so AI assistants you run can query scan results. It never connects outward, accepts no remote connections, exposes read-only tools, and stops when you stop it or close the app.
